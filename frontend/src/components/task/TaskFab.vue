...
const runningCount = computed(() => activeTasks.value.filter((t) => t.status === 'running').length)
const pendingCount = computed(() => activeTasks.value.filter((t) => t.status === 'pending').length)
const failedCount = computed(() => activeTasks.value.filter((t) => t.status === 'failed').length)
...
function formatBytes(bytes: number): string {
  ...
  return idx === 0 ? `${value} ${units[idx]}` : `${value.toFixed(2)} ${units[idx]}`
}
...
<template>
  ...
  <NDrawer v-model:show="showDrawer" :width="520" placement="right">
    <NDrawerContent :native-scrollbar="true">
      ...
      <NSpin v-if="loading && activeTasks.length === 0" :show="true" style="padding: 24px" />

      <NEmpty v-else-if="activeTasks.length === 0" description="当前无活跃任务" style="padding: 32px" />

      <div v-else class="task-list">
        <div v-for="task in activeTasks" :key="task.id" class="task-item" :class="{ 'is-running': task.status === 'running' }">
          <div class="task-item-header">
            <NTag size="small" :type="statusTagType(task.status)">
              {{ statusLabel(task.status) }}
            </NTag>
            <span v-if="task.status === 'pending' && pendingPosition.has(task.id)" class="queue-pos">
          ...
          <div class="task-item-body">
            ...
            <span v-if="task.releaseTag" class="task-tag">{{ task.releaseTag }}</span>
            <span v-if="task.assetName" class="task-asset">{{ task.assetName }}</span>
          </div>
          <div v-if="task.status === 'running'" class="task-progress">
            <div class="progress-line">
              <NProgress
                type="line"
                :percentage="hasDownloadProgress(task) ? progressPercent(task) : 100"
                :show-indicator="false"
                :height="6"
                status="warning"
                :processing="!hasDownloadProgress(task)"
              />
              <span v-if="hasDownloadProgress(task)" class="progress-label">{{ progressLabel(task) }}</span>
            </div>
          </div>
...
@keyframes fab-pulse {
  0%, 100% { box-shadow: 0 4px 12px rgba(31, 111, 235, 0.35); }
  50% { box-shadow: 0 4px 20px rgba(31, 111, 235, 0.65); }
}
